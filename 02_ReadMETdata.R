...
#Edited: 11/29/15 to get minimum temperature
...
min_temp1 = min(Sep_11$AirTC_Avg)
Sep_11 = as.data.frame(cbind(met_date1[1], met_avg_temp1[1], min_temp1[1]))
...
min_temp2 = min(Sep_18$AirTC_Avg)
Sep_18 = as.data.frame(cbind(met_date2[1], met_avg_temp2[1], min_temp2[1]))
...
min_temp3 = min(Sep_25$AirTC_Avg)
Sep_25 = as.data.frame(cbind(met_date3[1], met_avg_temp3[1], min_temp3[1]))
...
min_temp4 = min(Oct_02$AirTC_Avg)
Oct_02 = as.data.frame(cbind(met_date4[1], met_avg_temp4[1], min_temp4[1]))
...
min_temp5 = min(Oct_09$AirTC_Avg)
Oct_09 = as.data.frame(cbind(met_date5[1], met_avg_temp5[1], min_temp5[1]))
...
min_temp6 = min(Oct_16$AirTC_Avg)
Oct_16 = as.data.frame(cbind(met_date6[1], met_avg_temp6[1], min_temp6[1]))
...
min_temp7 = min(Oct_23$AirTC_Avg)
Oct_23 = as.data.frame(cbind(met_date7[1], met_avg_temp7[1], min_temp7[1]))
...
min_temp8 = min(Oct_31$AirTC_Avg)
Oct_31 = as.data.frame(cbind(met_date8[1], met_avg_temp8[1], min_temp8[1]))
...
min_temp9 = min(Nov_06$AirTC_Avg)
Nov_06 = as.data.frame(cbind(met_date9[1], met_avg_temp9[1], min_temp9[1]))
...
min_temp10 = min(Nov_13$AirTC_Avg)
Nov_13 = as.data.frame(cbind(met_date10[1], met_avg_temp10[1], min_temp10[1]))
...
names(main_with_met)[names(main_with_met)=="V2"] <- "Avg_Temp_degC"
names(main_with_met)[names(main_with_met)=="V3"] <- "Min_Temp_degC"

main_with_met$Avg_Temp_degC = as.numeric(as.character(main_with_met$Avg_Temp_degC))
main_with_met$Min_Temp_degC = as.numeric(as.character(main_with_met$Min_Temp_degC))
save(main_with_met, file = "main_df.RDA")

#PLOT AVG TEMP VS. TIME
load("~/B2-Honors-Proj/main_df.RDA")
plot_met_time = ggplot(main_with_met, aes(Date, Avg_Temp_degC))
...
load("~/B2-Honors-Proj/main_df.RDA")
plot_mintemp_time = ggplot(main_with_met, aes(Date, Min_Temp_degC))
plot_mintemp_time + geom_point(aes(size = 5)) + 
